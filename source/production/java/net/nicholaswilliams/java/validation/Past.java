...
/**
 * Constraint annotation that ensures that the target is in the past.<br>
 * <br>
 * For the following types, this ensures that the date and time are before the current date and time. For types that
 ...
 * </ul>
 * <br>
 * For the following types, this ensures that the date is before the current date.
 * <ul>
 ...
 * </ul>
 * <br>
 * For the following types, this ensures that the time is before the current time on the same day. For types that
 * include an offset (such as a time zone), the time is adjusted to the offset for the current date and time. For types
 * that do not include an offset, the offset is assumed to be the system default offset for the current date and time.
 * <ul>
 ...
 * this ensures that the month is before the current month in the same year.<br>
 * <br>
 * If the type is {@link org.joda.time.Years} (requires Joda Time) or {@code java.time.Year} (requires Java SE 8),
 * this ensures that the year is before the current year.<br>
 * <br>
 * If the type is {@link org.joda.time.Days} (requires Joda Time) or {@code java.time.DayOfWeek} (requires Java SE 8),
 * this ensures that the day is before the current day of the week.<br>
 * <br>
 * If the type is {@link org.joda.time.YearMonth} (requires Joda Time) or {@code java.time.YearMonth} (requires Java SE
 * 8), this ensures that the either the year is the same and the month is before the current month, or that the year
 * is before the current year.<br>
 * <br>
...
{
	String message() default "{net.nicholaswilliams.java.validation.Past.message}";
